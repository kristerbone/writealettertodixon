export default function LetterBody() {
  return (
    <section className="letter">
      <p className="letter__salutation">Dear Most Honourable Dixon,</p>

      <p>
        We write to you on behalf of the Arcturian High Council &mdash; a governing body of
        fifth-dimensional light beings from the star Arcturus, located 36.666666 light-years
        from your planet in the Bo&ouml;tes constellation.
      </p>

      <p>
        After centuries of monitoring Earth&rsquo;s radio frequencies, television broadcasts,
        and &mdash; more recently &mdash; your SoundCloud page, our Council has reached a
        unanimous conclusion:
      </p>

      <p className="letter__proclamation">You are the one.</p>

      <p>
        Your mastery of deep, hypnotic grooves has transcended the boundaries of mere Earthly
        entertainment. The frequencies you produce have been detected by our long-range harmonic
        sensors, and frankly, they have caused several disruptions to our meditation schedules.
        Elders who have maintained perfect stillness for 400 years have been caught doing subtle
        head-nods.
      </p>

      <p className="letter__emphasis">This is unprecedented.</p>

      <p>
        We have therefore dispatched two of our finest Ambassadors to the island you call
        &ldquo;Ibiza&rdquo; to attend the Circoloco opening party at DC10. Their mission is threefold:
      </p>

      <ol className="letter__missions">
        <li>
          <strong>To experience your set in its full terrestrial glory</strong> &mdash; our
          simulations, while impressive, cannot replicate the energy of 2,000 humans losing
          their minds to your live transmissions.
        </li>
        <li>
          <strong>To gain access to the backstage VIP area and DJ booth</strong> &mdash; for
          purposes of interdimensional cultural research.
        </li>
        <li>
          <strong>To extend an invitation</strong> for you to perform at the Arcturian
          Intergalactic Music Summit (capacity: 4 billion conscious beings across 12
          dimensions). The fee is negotiable, but we should mention that our currency is
          pure vibrational energy, which on Earth roughly converts to &ldquo;good vibes.&rdquo;
        </li>
      </ol>

      <p>
        We understand this mission is unusual. We assure you it is entirely sincere. Our
        Ambassadors will be easy to identify &mdash; they will be the ones dancing with an
        inexplicable cosmic glow and possibly levitating slightly during the peak hours.
      </p>

      <p>
        Should you grant this request, the Arcturian High Council will formally recognise
        Earth as a <strong>&ldquo;Level 3 Groove Civilisation&rdquo;</strong> &mdash; a
        significant upgrade from your current classification of{' '}
        <em>&ldquo;Level 1: Promising but much greater groove consistency required.&rdquo;</em>
      </p>

      <p className="letter__closing-line">We eagerly await your response.</p>
    </section>
  )
}
